...
// Pro-only industries are tracked in a separate constant:
const PRO_INDUSTRIES = ['AI Agent Development']
// Industries in this array show blurred agent cards to free users.
// Clicking any card in a Pro industry triggers the upgrade modal.
```
...
> Note: `proOnly` defaults to `false`. When `true`, the entire industry this agent belongs to will be added to the `PRO_INDUSTRIES` array. Pro-only industries show blurred cards to free users. Requires Rob's approval.
